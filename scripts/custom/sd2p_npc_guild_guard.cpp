...
using namespace nsSD2P::nsGuildGuard;
...
{
    npc_guild_guard_base_ai(Creature * pCreature, uint32 GuildId)
        : ScriptedAI(pCreature), m_GuildId(GuildId) {}

    const uint32 m_GuildId;
    ...
    void MoveInLineOfSight(Unit * pWho)
    {
        if (!m_creature->IsHostileTo(pWho))
        {
            ...
            if (m_GuildId == ((Player*)pWho)->GetGuildId())
                return;
        }
...
{
    npc_guild_guard_ai(Creature * pCreature, uint32 GuildId)
        : npc_guild_guard_base_ai(pCreature, GuildId)
    {
        Reset();
    }
...
CreatureAI* GetAI_npc_guild_guard(Creature * pCreature)
{
    ...
    if (pGarde)
        return new npc_guild_guard_ai(pCreature, pGarde->m_GuildId);
