...
using namespace nsSD2P::nsRecompense;
...
namespace
{
    ...
    void ActionAchat(Player * const pPlayer, Creature * const pCreature, const uint32 &id)
    {
        ...
        if (pPlayer->HasItemCount(pRec->m_Entry, 1, true))
        {
        ...
        for (ExtendedCost::VCost_t::const_iterator It = pExtCost->m_VCost.begin();
             It < pExtCost->m_VCost.end(); ++It)
        {
